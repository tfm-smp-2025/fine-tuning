...
def load_collection(collection_group: str, collection_name: str, texts: IndexedEntries):
    with Connection() as client:
        ...
        get_context().log_operation(
            level="INFO",
            message="Loading collection {} with {} elements".format(
                (collection_group, collection_name), len(texts)
            ),
        ...
        with collection.batch.fixed_size(
            batch_size=IMPORT_BATCH_SIZE, concurrent_requests=IMPORT_CONCURRENT_REQUESTS
        ) as batch:
...
def find_close_in_collection(
    collection_group: str, collection_name: str, reference: str
) -> list[RankedTerm]:
    """
