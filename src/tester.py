    ...
    for dataset_name in args.datasets:
        ...
        for translator in all_translators:
            if translator.model.model_name not in args.models:
                logging.info(
                    "SKIPPING, model {} not selected".format(
            ...
            for question in tqdm.tqdm(ds.get_test_data()):
                with logger.context(
                    ...
                    if sample_size is not None and dataset_counter > sample_size:
                        logging.info(
                            "Closing dataset after {} elements tested".format(
                                sample_size
                            )
                        )
                        break
                    ...
                    try:
                        if ontology:
                            ...
                            ctxt.log_operation(
                                level="INFO",
                                message="Expected query result: {}".format(
                                    expected_result
                                ),
                        ...
                        if ontology:
                            logging.info(
                                "TESTING query: {}".format(sparql_code_blocks[-1])
                            )
                            translator_result = ontology.run_query(
                                sparql_code_blocks[-1].content
                            )

                            test_success = check_if_equivalent_result(
                                translator_result, expected_result
                            )

                            ctxt.log_operation(
                                level="INFO",
                                message="Translated query result: {}".format(
                                    translator_result
                                ),
