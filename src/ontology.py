...
class Ontology:
    ...
    def run_query(self, query, quiet=False):
        ...
        return [binding for binding in ret["results"]["bindings"]]
    ...
    def iterative_listing(
        ...
        while True:
            if len(all_times) == 0:
                ...
                time_until_now = sum([td.total_seconds() for td in all_times])
                ...
                by_thousand = datetime.timedelta(
                    seconds=(time_until_now * 1000) / total_count
                )
                by_million = datetime.timedelta(
                    seconds=(time_until_now * 1000 * 1000) / total_count
                )
                time_info = f"last time: {all_times[-1]}; mean: {mean_time}; {by_element}/item; {by_thousand}/k; {by_million}/M; {len(known_results)} uniques"
            print(
                "\r\x1b[0K {} -> {} ({})".format(offset, offset + limit, time_info),
                end="\r",
                flush=True,
            )
        ...
        get_context().log_operation(
            level="DEBUG",
            message="Completed iterative SPARQL query with {} results: {}".format(
    ...
    ?s ?prop []
        ...
        return [prop["prop"]["value"] for prop in props]

    def get_superclass_of_class(self, class_uri) -> str:
        superclasses = self.run_query(
            f"""
    ...
    {comb_path_filter}
        ...
        for prop in results:
            if ":" in prop["subobj"]["value"]:
                ...
                subresults = list(
                    self.get_data_in_blank_node(
                        item_uri, path + [prop["subprop"]["value"]]
                    )
                )
    ...
    <{item_uri}> ?prop ?obj
        ...
        for prop in results:
            if ":" in prop["obj"]["value"]:
                ...
                subresults = list(
                    self.get_data_in_blank_node(item_uri, [prop["prop"]["value"]])
                )
...
WHERE {{
    ...
    def find_relations_outgoing_from_nodes(
        ...
        get_context().log_operation(
            level="INFO",
            message="Find relations outgoing from nodes: {} nodes & {} predicates".format(
                len(nodes), len(predicates)
            ),
    ...
    def get_all_properties_in_graph(self) -> PropertyGraph:
        raise Exception(
            "This operation is too expensive, probably you want to not use it. If you really need it use `._get_all_properties_in_graph`"
        )
    ...
    """
        ...
        graph = {
            "annotation_properties": {
                prop["prop"]["value"]: {} for prop in annotation_properties
            },
            "object_properties": {
                prop["prop"]["value"]: {} for prop in object_properties
            },
            "datatype_properties": {
                prop["prop"]["value"]: {} for prop in datatype_properties
            },
            "classes": {},
        ...
        for prop in class_data["obj_properties"]:
            g.add(
                (
                    class_uri,
                    rdflib.URIRef(
                        "http://codigoparallevar.com/tfm/ontology#objectProperty"
                    ),
                    rdflib.URIRef(prop),
                )
            )
