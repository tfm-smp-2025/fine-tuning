...
def to_qa_list(dataset: list | dict) -> list[QA]:
    ...
    assert isinstance(dataset, list), "Expected a list of data, found: {}".format(
        type(dataset)
    )
        ...
        if "corrected_question" in row and "sparql_query" in row:
            ...
            if isinstance(row["question"], list) and isinstance(row["query"], dict):
                for alt in row["question"]:
                    ...
                    assert (
                        "string" in alt and "language" in alt
...
class UnifiedDatasetLoader(DatasetLoader):
    def __init__(
        self, name: str, train_split: float = 0.8, rand_seed: Optional[int] = None
    ):
        self.name = name
        ...
        assert os.path.isfile(self.file_path), "Expected {} to be a file".format(
            self.file_path
        )
        self.data = None
